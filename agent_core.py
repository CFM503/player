            ...
            if m:  # 若正则匹配成功
                ...
                if not detected_type:  # 若尚未匹配到作业票类型
                    ...
                    if detected_type:  # 若成功匹配
                        ...
                        # ================= 视觉多模态大模型 局部网格补丁 =================
                        # 仅当对齐成功、识别为带气作业票且启用了 vision_brain 时触发
                        if detected_type == "带气作业票" and "aligned_" in os.path.basename(image_path):
                            if vision_brain is not None:
                                try:
                                    import cv2
                                    import base64
                                    safe_print("[Vision LLM] 启动带气作业票固定坐标网格裁剪与手写符号识别...")
                                    img_grid = cv2.imread(image_path)
                                    # 根据对齐后的 794x1030 绝对坐标裁剪红框网格区域
                                    crop_grid = img_grid[230:900, 500:750]
                                    crop_path = image_path + ".grid.jpg"
                                    cv2.imwrite(crop_path, crop_grid)
                                    
                                    with open(crop_path, "rb") as f:
                                        b64_grid = base64.b64encode(f.read()).decode()
                                    
                                    grid_prompt = (
                                        "请仔细观察这张25行5列的表格网格图片。每一行代表一个安全措施的检查结果，"
                                        "每一列代表不同角色（依次为：作业人, 现场负责人, 监理, 监护人, 现场负责人）的手写签字或打勾符号。\n"
                                        "请判断每一格里面的手写符号是对号（√）、叉号（×）、斜杠（\\）还是空白。\n"
                                        "要求：\n"
                                        "1. 请以纯文本列表格式输出，每行对应原图的一行（共25行）。\n"
                                        "2. 格式范例：\n"
                                        "第1条: 作业人(√), 现场负责人(√), 监理(×), 监护人(\\), 现场负责人(√)\n"
                                        "第2条: 作业人(空白), 现场负责人(√), 监理(空白), 监护人(空白), 现场负责人(空白)\n"
                                        "3. 只输出这25行的文字，绝对不要输出多余的解释，不要输出markdown代码块符号。"
                                    )
                                    
                                    resp_grid = vision_brain.client.chat.completions.create(
                                        model=vision_brain.model_name,
                                        messages=[{
                                            "role": "user",
                                            "content": [
                                    ...
                                    grid_md = resp_grid.choices[0].message.content.strip()
                                    
                                    append_text = f"\n\n--- 视觉大模型局部符号提取结果 ---\n{grid_md}\n----------------------------------\n"
                                    flat_text += append_text
                                    ocr_result += append_text
                                    AgentTools._last_ocr_raw = flat_text
                                    safe_print("[Vision LLM] 手写符号网格补丁识别完成并注入上下文！")
                                except Exception as e:
                                    safe_print(f"[Vision LLM] 网格符号识别异常跳过: {e}")
                            else:
                                # 纯本地 OpenCV 像素密度检测降级方案
                                try:
                                    import cv2
                                    import numpy as np
                                    import re
                                    safe_print("[OpenCV Fallback] 未配置视觉大模型，启用纯本地 OpenCV 像素密度交叉定位符号...")
                                    img_bgr = cv2.imread(image_path)
                                    img_gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
                                    bw = cv2.adaptiveThreshold(~img_gray, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 15, -2)
                                    
                                    # 读取带气作业票的所有条目
                                    # 因为要在当前类的方法内部使用，避免循环引入，从外部字典直接读取
                                    # agent_core 中全局的 STANDARD_MEASURES 已经被加载过了
                                    from agent_core import STANDARD_MEASURES
                                    measures = STANDARD_MEASURES.get("带气作业票", [])
                                    
                                    # 提取 OCR 中的文本和它的绝对坐标 Y
                                    lines_coords = []
                                    ...
                                    for idx, desc in measures:
                                        norm_desc = clean_str(desc)
                                        best_y, best_h = -1, -1
                                        # 第一轮严格匹配
                                        for txt, cy, ch in lines_coords:
                                            norm_txt = clean_str(txt)
