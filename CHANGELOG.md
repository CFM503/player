...
### 新增
- **新增多票型分流逻辑**：上传时在主面板提供作业票类型单选选项，支持选择“带气作业票”或“动火作业票”分流，默认选中“带气作业票”。
- **增加关键人员提取展示**：作业票中关键的负责人及作业人员信息（作业人员 `workers`、施工方现场负责人 `operator_leader`、监理人员 `supervisor`、项目公司监护人 `company_guardian`、带气现场负责人 `gas_leader`）增加提取字段，并统一在智能审批结果提示框 `stAlert` 的“关键人员及基础信息”列表中呈现。
- **引入外部对齐脚本支持**：在感知（OCR）阶段弃用旧版特征点匹配，全面接入外部脚本 `align_to_template.py`，采用自适应阈值轮廓查找算子来定位表格最外层边缘并透视拉平对齐到 `dq.png` 或 `dh.png`，在对齐失败时能够安全自动降级至 Resize 缩放兜底。

### 修复
- **修复智能审批框在重绘或侧边栏变动时自动消失的 UI 缺陷**：重构了 `frontend.py` 的渲染与状态维持逻辑。处理日志、结构化数据、OCR 原文及决策链报告现在会直接随本次结果存入 `st.session_state.results`。运行结束后通过 `st.rerun()` 触发持久化两栏重绘渲染，使得任何 UI 按钮交互或参数调整都不会再意外刷掉大屏结果及智能审批建议提示框。
- **规范签字裁剪图归档路径**：物理保存裁剪出的签字图片至固定的 `archives/` 目录下，以便于后续流程的规范性溯源。
