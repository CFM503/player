...
### 新增
- **ocr9 手写模糊增补（非兜底）**：精确 MD5 之后，以 **IoU 门控 + aHash/L1 外观综合分** 对纠错记忆做有证据增补；无证据保留模型输出。入库写入 `box` / `ahash` / `crop_relpath`；admin 可开关「手写模糊增补」。
- **环境变量**：`OCR9_MEMORY_FUZZY`、`OCR9_FUZZY_IOU`、`OCR9_FUZZY_APP`、`OCR9_FUZZY_HAMMING_MAX`。

### 改进
- **数据看板列表**：展示 **地点**（动火=动火地点，带气=作业单位/场站）与 **发起人签字确认**；搜索支持票号/地点/发起人。
- **钉钉写入**：临时关闭一级双写 **test_demo_base2**，仅写 `test_demo_base`（代码已注释，可恢复）。

### 修复
- **看板等级显示双「特级」**：标题同时写了明文 `grade` 与 `render_record_badge`，改为只保留着色徽章一处。
...
### 改进
- **ocr9 默认 GPU**：工作台默认 `device=gpu`（侧栏设备、引擎 fallback、重置默认一致）。
- **点击卡顿**：去掉管理页/用户页假进度 `time.sleep` 与处理前空 `st.rerun`；管理页 JS `setInterval` 只挂一次，避免越点越卡。

## [3.15.10] - 2026-07-15

### 修复
- **ocr9 / ocr10「重置为默认参数」无效**：带 `key` 的滑标以 `session_state` 为准会忽略 `value=`；改为 `on_click` 在渲染前清旧 key、换 `ui_gen` 并预填默认值，界面真正回到默认。
...
### 修复
- **动火「发起人签字确认」裁剪 ROI**：`sign_crop` 调整为 `(635, 165, 320, 210)`（1000×1414 画布），对准右上合并签字格。

### 改进
- **数据看板**：去掉「高频问题 Top5」；KPI 与记录详情状态由「有异常」改为「**发现漏填**」。
- **侧栏导航**：去掉「业务 / 模型训练」分组标题及 logo 下品牌文案，扁平展示「提交作业票 / 管理测试 / OCR 训练页」。

## [3.15.07] - 2026-07-15

### 新增
- **`ocr5.py` 动火勾选适配（与带气分离）**：
  - 带气：25×5 列（作业人/施工方/监理/监护人/带气现场负责人），画布 1052×1487
  - 动火：21×1 列「确认」，画布 1000×1414；独立 `FIRE_REF_Y_LINES` / `FIRE_REF_X_BOUNDS`
  - 共用四分类 `classify_mark`（✓ / × / \ / 空白）；`--ticket-type` 显式分路
- **主流程动火也调用 ocr5**：对齐后 `--ticket-type 动火作业票`；`parse_fire_measure_grid` 解析单列确认格；缺失再回退行内启发式

### 改进
- **`ocr7.py` 去表格线票型分离**：`LINE_PROFILES` 带气/动火独立横竖线比例、C 值、blockSize、inpaint 半径；`--ticket-type`；`agent_core` / `ocr5` 调用时传入当前票型

### 说明
- 带气 25×5 与动火 21×1 **永远分路**，禁止共用网格几何与去线默认阈值

## [3.15.06] - 2026-07-15


### 资产
- **动火模板 `template/dh.png` 固定为 1000×1414**：与业务规范画布一致（不再使用 2000×2827 高清再缩放路径）；对齐后可不二次插值，利于坐标稳定。

### 改进
- **`align_to_template.py` 带气/动火参数完全分离**：`--ticket-type`（或按 `dq.png`/`dh.png` 推断）；独立轮廓面积比、ORB 特征数/匹配阈值/源图缩放上限/排序加分，禁止混用一套默认值。
- **主流程对齐传票型**：`agent_core` 调用对齐脚本时显式传入当前票型。
- **签字裁剪 ROI 分票型**：`TICKET_TEMPLATE_SPEC.sign_crop` — 带气 `(670,230,280,170)`、动火 `(640,220,270,160)`；归档 `cropimage` 不再写死带气坐标。

### 说明
- 对齐取点仍自动（ORB/四点），无需手动画点配置；若动火版式相对旧 ROI 有偏移，仅需改动火 `sign_crop`。

## [3.15.05] - 2026-07-15


### 新增
- **动火作业票独立流水线**：UI 可选「带气作业票 / 动火作业票」；用户选择锁定全程，禁止 OCR 跨票型改写。
- **模板分离**：恢复 `template/dh.png`（动火 1000×1414）与 `template/dq.png`（带气 1052×1487）按票型精确匹配，禁止交叉试配。
- **动火措施 21 条单列勾选**：OCR 行内 ✓/× 解析；**不调用 ocr5**；与带气 25×5 网格完全分离。
- **动火字段齐全**：作业票编号、动火单位、动火地点、动火内容、动火时间、动火方式、动火人姓名及证书编号、采样检测、动火等级、动火人员、施工方现场负责人、监理人员、项目公司监护人员、动火现场负责人(项目公司)、动火现场负责人。
- **数据库分表**：带气 → `hse_gas_work_tickets`；动火 → `hse_hot_work_tickets`；废弃并删除混用表 `hse_fire_work_tickets`（不保留历史，重新录入）。
- **带气表结构对齐业务变量**：作业票编号/单位/内容/时间/作业人/日期/完工时间/作业等级/发起人签字/签批五列 + 措施 JSON + 审批字段，入库一一对应；看板完整展示。
- **LLM 提示词分路**：动火 / 带气各自 system prompt，字段语义不混用。

### 改进
- **展示组件**：KPI/审批卡按票型显示字段（动火不展示带气五列负责人）。
- **动火等级高低**：**特级最高，二级居中，一级最低**（与带气「一级最高」分离）；钉钉仅动火**特级**走双写。
- **数据看板**：可按全部/带气/动火筛选，删除指定分表记录。

### 修复
- **带气签批五列人名漏取**：
  - **监理人员**：剥离「已确认」粘连（如 `已确认于华` → `于华`），禁止整段因噪声丢弃。
  - **作业人员**：支持数字混汉字（`1 佛` → `佛`）；过滤日期残片/套话伪名；列 x 窗收紧防串列。
  - **带气现场负责人**：y 窗覆盖页脚；列 x 窗收紧；`带气现场负责人签字` 旁人名回退；文本回退不强制冒号。

## [3.15.04] - 2026-07-15


### 新增
- **主应用嵌入模型训练页**：`frontend.py` 侧栏分组为 **业务**（提交作业票 / 管理测试）与 **模型训练**（OCR文字训练 / OCR5勾选训练）。
- **训练页入口**：`train_ocr_ui.py`（封装 ocr9 → 服务生产 `ocr.py`）、`train_ocr5_ui.py`（封装 ocr10 → 服务生产 `ocr5.py`），路由 `/train-ocr`、`/train-ocr5`。

### 改进
- **ocr9 / ocr10 可嵌入多页**：`set_page_config` 在已由 frontend 配置时跳过，避免二次调用报错；标题/说明对齐生产链路（ocr.py 文字、ocr5 勾选格）。
- **宣传 PPT**：痛点页提前；闭环时长文案由「15 秒」统一为「60 秒」；「一句话，跑通全流程」改为「自动化跑通全流程」等（`promo_assets/create_promo_ppt.js` 与根目录/宣传资产 PPT）。

## [3.15.03] - 2026-07-14


### 业务
- **审批改为「漏填审核」**：不再做隐患整改类建议；漏填 0 项 → **自动通过**；有漏填 → **待审批/人工介入**（不驳回）。审批文案：`【填写齐全】` / `【发现漏填】`。
- **钉钉多维表路由按作业等级**：二级仅写 **`test_demo_base`**；一级双写 **`test_demo_base` + `test_demo_base2`**（按 base 名称匹配，不按下标）。
- **文案清理**：全程序去掉「隐患」「禁止放行」「暂缓作业」等界面/业务用语（纸质票第 25 条标准原文保留供 OCR 对齐）。

### 修复
- **管理页 Logo 不在最上**：移除会把 `stSidebarHeader` 挪到 HSE 标题下的 `moveHeader` 脚本；与用户页一致由 `frontend.st.logo` 固定侧栏顶。
- **管理页审批结果区**：处理完成后左栏「查看原图」下展示 KPI + 审批建议；会话重绘可全宽补画。

### 改进
- **LLM 结构化提取**：全厂商统一文本模式；DeepSeek 关闭 thinking（`extra_body.thinking.disabled`）；单次请求 + 本地 JSON 解析增强。
...
- **Web 运行日志中文乱码**：子进程（对齐等）在 Windows 上 GBK 输出被 UTF-8 误读；`run_python_script` + 稳健解码，`align_to_template` 强制 UTF-8 stdio。
- **依赖检查 sklearn FAIL**：`scikit-learn` 写入 `requirements.txt` 与 `check_deps` 正式依赖（ocr10 特征分类 / ocr5 加载 pkl）。

### 改进
- **ocr10 预览**：每页格数可选 25/50/75/100/125；预测标签按类别着色（绿✓ 红× 蓝\ 灰-）；格子左上角斜杠标签改为票面图例 **`\`**（非 `/`）。
- **对齐链路**：`align_to_template` 轴对齐模板角点 + 四点/ORB 择优；主程序透出 Align 匹配/内点日志。
- **去除动火残留**：删除 `template/dh.png` 及业务侧动火交叉逻辑（仅保留带气票路径）。

### 数据
- **ocr10 标注样本同步**：勾选格 check/slash 裁剪与 `labels.jsonl` / 纠错记忆更新（供多机训练；主程序仍须规则搜索/sklearn 导出后生效）。

## [3.14.68] - 2026-07-13

### 新增
- **ocr9.py**：文字 OCR 交互式标注/训练工作台（Streamlit，默认端口 8502）；即时预览、逐项入库、纠错记忆、训练任务登记、`-h` 说明。
- **ocr10.py**：勾选格 √/×/\ /空白 训练工作台（默认端口 8503）；25×5 预览、逐格入库、规则网格搜索、可选 sklearn、导出给 ocr5。
- **多机协同训练**：`ocr_train_workspace/`（ocr9）、`ocr_mark_workspace/` + `ocr5_mark_params.json`（ocr10）纳入版本库约定；忽略 raw/runs 大文件。
- **文档**：`OCR训练说明.md`、`ocr9主程序使用步骤.md`、`产品说明.md`（含人工介入=钉钉 MCP）。

### 改进
- **ocr5 网格线检测**：多阈值/形态学/比例标定；**先检线再去表格线**；`get_x_bounds` 随图宽缩放；可加载 `ocr5_mark_params.json` / `ocr5_mark_model.pkl`。
- **ocr9/ocr10 侧栏**：↺ 重置默认参数（换 ui_gen 强制滑标回归）；滑标变更可自动刷新预览。
- **带气业务链路**（3.14.63–3.14.67 累计）：25×5 完整性、作业等级一级/二级、禁止静默兜底、审批路径钉钉人工介入文案等。

## [3.14.67] - 2026-07-13

### 文案
- **统一「人工介入」语义**：界面与日志写明 = 经 MCP 推送钉钉 AI 表格；`待审批`/`已驳回` 旁注审批路径。
- **作业等级替代「风险」展示**：KPI/汇总表/历史详情用「作业等级」；「动火人」改为「作业人姓名及证书编号」。
- **L3 路由文案**：主管审批 → `钉钉人工介入` / `MCP 推送钉钉 AI 表格`。

### 文档
- **新增 `产品说明.md`**：带气作业票功能/优势/五阶段实现/合规校验与人工介入（钉钉 MCP）说明。

### 本版一并发布（相对 v3.14.62）
- **3.14.63–3.14.64**：反思完整性（25 项×5 列 √/×/\、日期签名）；ocr5 四分类，空白不再当叉号。
- **3.14.65**：表头作业等级一级/二级（一级危险最高）驱动展示与 L3 路由。
- **3.14.66**：禁止业务静默兜底，失败报错/记异常。

## [3.14.66] - 2026-07-13

### 规范
- **禁止业务兜底**：失败处改为报错/记异常，不静默用默认值放过。关键变更：符号无法识别→blank；作业等级未识别→完整性失败+禁止作业（禁止当二级）；发起人签字禁止 LLM 回填；审批建议 LLM 失败→系统报错文案（禁止同意模板）；ocr5 网格线非 26 条→抛错（禁止默认坐标）；ocr5 子进程失败→中断流水线。原兜底点均加 `【禁止兜底】` 注释说明。

## [3.14.65] - 2026-07-13

### 改进
- **带气作业票风险等级 = 表头作业等级**：从 OCR 标题「作业等级：一级/二级」提取（一级危险最高），写入 `risk_level`；审批建议、KPI、归档、钉钉均展示一级/二级，禁止再写重大/较大/一般/低风险。
- **带气 L3 路由**：有异常/天气禁止 → 驳回；无异常时一级 → 主管审批，二级 → 可自动通过。

## [3.14.64] - 2026-07-13

### 改进
- **带气安全措施按 25×5 格校验填写完整性**：每项 5 列（作业人/施工方现场负责人/监理/监护人/带气现场负责人）须填图例三态之一——落实 **√**、未落实 **×**、不适用 **\\**；空白格记漏项。`ocr5` 改为四分类输出 `(✓)/(x)/(\)/(-)`，空白不再误标为叉号。
- **数据结构**：`SafetyMeasureItem.column_marks` 记录五列归一标记（check/cross/slash/blank）；sanitize 优先解析 ocr5 网格；反思阶段按空白格明细拦截。

## [3.14.63] - 2026-07-13

### 改进
- **反思阶段数据完整性（仅带气，不与动火交叉）**：① 25 项安全措施全部填写、无漏项；② 所有日期与签名全部填写、无漏项。两项均通过则完整性校验通过；任一项漏项触发重试并记入问题。
- **完工时间提取补强**：LLM 结构化提示词增加 `completion_time`；sanitize 阶段支持从 OCR「完工时间」栏回退提取。

## [3.14.62] - 2026-07-13

### 修复
- **签批五列坐标提取串位**：列头匹配改为整词（可带冒号），禁止 `startswith`，避免正文「作业人员严禁…」与「带气现场负责人签字」误当列头导致 y 窗偏移；签名候选支持 1～4 字（保留 OCR 单字如监理「华」）；按「签名 x ↔ 列头 x」距离全局贪心一对一分配。
- **签批五列禁止 LLM 兜底**：`operators` / `construction_leader` / `supervisor` / `company_monitor` / `gas_leader` 仅写坐标提取结果，识别不到则为空，杜绝 LLM 猜列造成的张冠李戴。
...
### 修复
- **修复签批区域5列签名映射错位的严重 Bug**：OCR 将签批区域的列头和手写签名拼成一行纯文本，LLM 无法从中恢复列位置关系，导致作业人员、施工方现场负责人、监理人员、项目公司监护人、带气现场负责人等字段张冠李戴。新增 `_extract_sign_columns` 方法，利用 OCR 坐标数据中的 x 值做最近邻列匹配，彻底解决列错位问题。
- **增强 LLM 提取 Prompt**：在 `extract_sheet_json` 的 system_prompt 中增加签批区域5列提取规则的详细说明，指导 LLM 按列头从左到右的顺序一一对应提取签名。

### 新增
- **审批建议归档**：在 `_act` 执行阶段新增步骤⑦，将审批建议、核心要素及 LLM 提示词归档为 `{票号}_{时间戳}_审批建议.txt` 文件，与 OCR 文本、原图、对齐图等文件统一保存在 `archives/{日期}/` 目录。
- **LLM 提示词归档**：在 `_generate_approval` 中缓存发给 LLM 的提示词，归档时一并写入审批建议文件末尾，便于追溯和审计。

## [3.14.58] - 2026-07-12

### 修复
- **修复审批建议偶发性丢失的严重 Bug**：当 LLM（如 DeepSeek R1 等推理模型）将审批建议的完整内容包裹在 `<think>` 标签内返回时，`clean_thinking()` 会将全部有效内容清洗为空字符串，导致 `approval_opinion` 为空，前端 `if d.approval_opinion:` 判定失败，审批建议框静默消失。现已在 `_generate_approval` 方法中增加空值防护，当 LLM 返回被清洗为空时自动回退至模板兜底生成审批建议，确保审批建议永不缺失。

## [3.14.57] - 2026-07-11

### 规范
- **变量声明置顶规范**：在 `cropimage.py` 脚本头部和 `agent_core.py` 的 `AgentTools` 类头部统一声明了核心全局变量、常量及类静态属性。
- **中英双语注释**：为置顶声明的这组变量提供了中英文对照的详细功能注释，提升了代码的规范度与可读性。

## [3.14.56] - 2026-07-11

### 重构
- **收拢所有图片处理与 OCR 至第二阶段**：将原本在第三阶段（语义校验环节）执行的发起人手写签字 OCR 文字识别，完全提取提前合并至第二阶段（归档环节 `_archive`）中完成。
- **解耦第三阶段的图像计算依赖**：重构 `AgentTools.extract_filler_name`，使其在第三阶段仅直接读取第二阶段预提取并缓存好的静态类变量 `_last_approver_name`，彻底避免了第三阶段及之后再次运行任何图片裁剪或 OCR，实现了纯净的阶段职责解耦。

## [3.14.55] - 2026-07-11

### 新增
- **新增独立图片裁剪工具 `cropimage.py`**：支持 CLI 命令行模式及 `-h`/`--help` 参数帮助描述，允许指定 `-x`, `-y`, `-w`（宽度）与 `--height`（高度）参数进行图形物理裁剪并生成图片文件。
- **第二阶段集成签字裁剪功能**：在第二阶段感知落地的归档流程（`_archive` 环节）中，系统会自动通过 `subprocess` 调用 `cropimage.py`，对对齐图片的发起人签字区域（坐标 `[670, 230, 280, 170]`）进行局部裁剪，并将提取出的手写签字图独立归档保存为 `[prefix]_签字.png` 副本。

## [3.14.54] - 2026-07-11

### 重构
- **重构感知阶段图片归档流**：将保存原图、保存对齐图和保存去网格图的物理归档操作（原 `_archive`）合并提前到第二阶段（感知阶段 `_perceive`）中，确保图片在完成识别后立刻同步落地到本地归档目录。
- **统一支持去表格线网格化功能**：对“动火作业票”与“带气作业票”的对齐图像统一进行 `ocr7` 表格线擦除（去网格化）预处理，并输出独立的 `[prefix]_对齐图去表格化.png` 副本，统一并丰富了所有票据的处理管道。
...
### 修复
- **依赖自检与智能安装优化**：在 `START.bat` 启动脚本中新增对 `mcp`、`httpx` 和 `httpx-sse` 缺失依赖的自检检测。
- **添加镜像安装故障降级逻辑**：将 `START.bat` 中 pip 批量安装机制优化为按包循环执行，若使用清华镜像源（Tsinghua Mirror）安装某依赖失败（如镜像源暂无 `mcp` 库），会自动对当前包降级并使用 PyPI 官方源重新下载安装，确保网络环境不佳或镜像源缺失时的安装高成功率与执行效率。
...
### 新增
- **新增表格线擦除工具 `ocr7.py`**：基于局部自适应高斯二值化（Adaptive Gaussian Thresholding）及交叉膨胀（Cross-Dilation）机制，提供高精度的去表格线及图像 Inpaint 修复能力。

### 修复
- **优化表格线去除算法（支持阴影与倾斜）**：将 `ocr7.py` 中的全局二值化重构为自适应高斯二值化（`BlockSize=51`, `C=2`），并在横纵线条提取上分别引入方向性预膨胀，彻底解决了手机拍照阴影导致的大白块遮罩问题，以及对齐图 1~2 度倾斜导致表格线无法去除的问题。
- **解决对齐变形问题**：纠正了之前版本中 `template/dq.png` 误被 `dh.png`（动火票）覆盖的错误，回滚恢复了正确的「带气作业票」模板，使对齐特征内点数由 20 大幅提升至 105，彻底消除了形变。
- **实现 `去表格化.png` 自动归档**：在 `agent_core.py` 的 `_archive` 归档流中补充了对 `[aligned_file]去表格化.png` 文件的拷贝归档逻辑，确保最终 `archives` 目录下能同步查看到 `[prefix]_对齐图去表格化.png`。

### 变更
- **升级日志时间戳为北京时间**：将 Web 界面右侧终端控制台（`.hlog` 容器）的相对耗时计算替换为基于东八区绝对北京时间的 `[HH:MM:SS]`，提高日志的审计合规价值。

## [3.14.49] - 2026-07-10

### 修复
- **优化 `ocr7.py` 表格线去除算法（支持阴影与轻微倾斜）**：针对手机拍照时常见的光照不均、阴影干扰以及透齐对齐图时产生的 1~2 度轻微倾斜变形：
  1. 将全局 Otsu 二值化重构为**局部自适应高斯二值化（Adaptive Gaussian Thresholding）**，并调优 BlockSize=51, C=2 极小容差参数，确保提取出高亮区极细、极淡的表线。
  2. 引入**交叉膨胀（Cross-Dilation）**预处理机制：对横向开运算，将图片在垂直方向预膨胀以兼容微弱倾斜的线条；对纵向开运算，将图片在水平方向预膨胀；开运算提取线条后再腐蚀复原，使稍有倾斜的表线也能 100% 被捕获擦除，且无大面积白块遮挡。
...
### 修复
- **修复非 NVIDIA 显卡硬件下的 GPU 虚假降级漏洞**：修正了 `frontend.py` 中的回退逻辑。修复前，如果在无 NVIDIA 环境（如 Intel 核心显卡）下勾选 GPU 加速，UI 会发出“降级为 CPU”的警告，但底层参数并未被覆盖重置。修复后，增加了强制修改 `ocr_device = "cpu"` 的链路拦截，杜绝了向底层引擎错误下发 GPU 指令引发的环境兼容问题。
