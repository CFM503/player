...
- **3.14.66**：禁止业务静默兜底，失败报错/记隐患。

## [3.14.66] - 2026-07-13

### 规范
- **禁止业务兜底**：失败处改为报错/记隐患，不静默用默认值放过。关键变更：符号无法识别→blank；作业等级未识别→完整性失败+禁止作业（禁止当二级）；发起人签字禁止 LLM 回填；审批建议 LLM 失败→系统报错文案（禁止同意模板）；ocr5 网格线非 26 条→抛错（禁止默认坐标）；ocr5 子进程失败→中断流水线。原兜底点均加 `【禁止兜底】` 注释说明。

## [3.14.65] - 2026-07-13

### 改进
- **带气作业票风险等级 = 表头作业等级**：从 OCR 标题「作业等级：一级/二级」提取（一级危险最高），写入 `risk_level`；审批建议、KPI、归档、钉钉均展示一级/二级，禁止再写重大/较大/一般/低风险。
- **带气 L3 路由**：有隐患/天气禁止 → 驳回；无隐患时一级 → 主管审批，二级 → 可自动通过。
...
### 改进
- **反思阶段数据完整性（仅带气，不与动火交叉）**：① 25 项安全措施全部填写、无漏项；② 所有日期与签名全部填写、无漏项。两项均通过则完整性校验通过；任一项漏项触发重试并记入隐患。
